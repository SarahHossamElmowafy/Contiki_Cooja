...
// Implementation of Box Muller_transform to generate gaussian distribution samples
// "mu" is mean, "segma^2" is variance

float gaussian(double mu, double sigma){
	double epsilon = 0.00001;
	// Generate two random numbers and make sure that the first one is greater than epsilon
	double u1, u2;
	do
	{
		u1 = random_value(0,1);
