...
#define COLLECTION_INTERVAL (30 * CLOCK_SECOND)
...
PROCESS_THREAD(sensor_process, ev, data) {
	...
	PROCESS_BEGIN();
	simple_udp_register(&udp_connection, UDP_SENSOR_PORT, NULL, UDP_CONTROLLER_PORT, udp_rx_callback);
	...
	while(1) {
		...
		sensor.temperature = read_temperature(additive_noise, amplification_factor, fading_factor);
		
		additive_noise = get_additive_noise();
		sensor.humidity = read_humidity(additive_noise, amplification_factor, fading_factor);
		additive_noise = get_additive_noise();
		sensor.pressure = read_pressure(additive_noise, amplification_factor, fading_factor);
		if(NETSTACK_ROUTING.node_is_reachable() && NETSTACK_ROUTING.get_root_ipaddr(&dest_ipaddr)) {
			...
			simple_udp_sendto(&udp_connection, &sensor, sizeof(sensor), &dest_ipaddr);
